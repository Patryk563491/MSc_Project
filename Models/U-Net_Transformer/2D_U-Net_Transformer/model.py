...
class SelfAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        Returns:
            attn_output (torch.Tensor): Output tensor after applying self-attention (b, c, h, w)
        """
        ...
        # Transpose query and value for matrix multiplication
        q_flat = q_flat.transpose(1, 2)  # shape: (b, h*w, c)
        v_flat = v_flat.transpose(1, 2) # shape: (b, h*w, c)
        
        # Scaled dot-product attention
        attn_scores = torch.matmul(q_flat, k_flat)  # (b, h*w, h*w)
        ...
        attn_output = torch.matmul(attn_weights, v_flat)  # (b, h*w, c)
        attn_output = attn_output.transpose(-2, -1)  # (b, c, h*w)
        
        # Unflatten the output
        attn_output = attn_output.reshape(b, c, h, w)  # (b, c, h, w)
